...
A levelsio-style PHP micro-stack template. Raw PHP, SQLite, no framework,
no build step. Clone it, copy a config file, and you have passwordless auth,
Stripe payments, and a deploy pipeline ready to go.

## What it is

A starting point for a small subscription web app
that one person can run on one cheap VPS.
It favors plain functions, prepared statements, and boring infrastructure over abstractions.
Everything you need fits in a handful of files you can read in an afternoon.
...
Open http://127.0.0.1:8000. With the default config,
...
Feature set and philosophy inspired by Pieter Levels' book _MAKE_
(https://readmake.com).
